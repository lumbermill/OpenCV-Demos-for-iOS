...
#import "CannyMonitor.h"

@implementation CannyMonitor
...
- (cv::Mat)convert:(cv::Mat) src_img{
    ...
    // コントラストを上げる
    cv::equalizeHist(work_img, work_img);
    
    // ぼかし
    //cv::medianBlur(work_img, work_img, 9);
    
    // 入力画像，出力画像，閾値，maxVal，閾値処理手法
    //cv::threshold(work_img, work_img, self.gain*255, 255, cv::THRESH_TOZERO);
    // スライダ値によって閾値を変更
    ...
    cv::Canny(work_img, work_img, self.gain*255, self.gain2nd*255, 3);
    // スライダ値によって引数を変更
